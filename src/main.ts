import { List, Range, setIn } from "immutable";
...
type GridState = List<List<CellState>>;

function toggleCell(state: GridState, point: Coords): GridState {
  const [x, y] = point;
...
function at(state: GridState, point: Coords): CellState {
  const [x, y] = point;
